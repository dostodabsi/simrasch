...
> - Long interest in education, development, **assessment**
  (Blair, 1940; Piaget & Inhelder, 1958).
...
1 person parameter, 1 item parameter
...
## Practices in the Field: Rasch **Application** {.smaller}
...
| Grube (2010)            | x     | 0.8-1.2 (Adams, 2000) + t | EAP/PV      | x   | x    | x   | ConQuest             |
| Heene (2007)            | x     | 0.8-1.2 (Wright, 2000)    | PSR/ISR     | -   | x    | -   | ConQuest, WS, FC, WM |
| Brown et al. (2010)     | x     | -                         | PSR         | -   | -    | -   | ConQuest             |

- Result: Typical approach: Fit **unidimensional Rasch model** in **ConQuest software**, remove items based on **infit statistics**
...
Degree of deviation from expected scores given the Rasch model, on level of the item.

<div class="centered">
$INFIT_i = \frac{\sum^2_{p=1}R^2_{pi}}{\sum^2_{p=1}VAR(X_{pi})}$
</div>

> - Reviewed articles: Cutoff mostly 0.8-1.2 . Exceeding items removed.

> - Result: Adequacy for model fit testing **strongly criticized** (Christensen & Kreiner, 2013; Heene et al., 2014; Smith, 1995; Smith, 1996; Smith et al., 1998)
    * Underpowered.
    * Should be applied only after investigation of **global model fit**.
    * Item removal might **reduce construct validity**.
    * **Conclusion: Item infit statistics are in general no adequate means to evaluate fit of the Rasch model.**
    
## Itemfit: Simulation I {.smaller}
...
- Dichotomous answer data from 2PL model
    * Violation: Overlapping ICCS/varying factor loadings
    * Typical item and sample sizes as in the 7 reviewed Rasch articles

- Result:
    * **[FIGURE: SIMULATION RESULTS FROM 9 ITEM/SAMPLE SIZE COMBINATIONS]**
    * Lack of power to detect overlapping ICCs.
    * Rasch wrong model but just some random items are dismissed.
    * Item fit statistics are not model fit statistics. The model cannot be rejected, only items.
    * **Conclusion: Model fit was not adequately tested in any of the articles.**
      
## Itemfit: Simulation II {.smaller}
...
- Typical theoretical model of scientific thinking in the reviewed articles
    * Typically 3-5 correlated facets of SR assumed
    * Simulation: 4 correlated factors (r: .3-.7). 1000 dichotomous answer matrices
    * What do itemfit statistics tell us when we fit a unidimensional Rasch model on these data?  
    
- Result:
    * **[FIGURE: SIMULATION RESULTS FROM 9 ITEM/SAMPLE SIZE COMBINATIONS]**  
    * No power of itemfit statistics to detect multidimensionality
    * Unsystematic elimination of random items
    * Unidimensional Rasch model: Shows low reliability similar to reviewed articles (about .5-.6)
    * **Conclusion: Simulated scenario yields results similar to reviewed articles. Might the simulated scenario reflect what is happening in the field?**

## Practices in the Field: **Interpretation** {.smaller}
...
- Conclusions in reviewed articles:
    * "*good model data fit, suitableness for measurement, fulfillment of psychometric affordances of Rasch model, valid treatment as a single scale*"
    * "*scale measures unitary construct, support for unidimensional theoretical model*"
    * **Conclusion: Theoretical models, modeling practices, and theoretical conclusions are inconsistent. Researchers engage in tautological reasoning that supports their theoretical models and instrument quality.**
...
- **Conclusion: The current practices are reductionist, inadequate, inconsistent, and might be detrimental for theory development in the field of Scientific Thinking**. *Why do researchers engage in these practices?*
...
##Discussion: Suggestions {.smaller}
...
> -  **Acknowledge the hypothesis testing perspective**
    ...
    * Martin L?f test (Christensen et al., 2002)
    ...
    * **R** software (eRm, MIRT, ltm packages)

> - **Match theoretical and statistical model**
...
##Discussion: Suggestions {.smaller .flexbox .vcenter}
...
- **Acknowledge complexity**
    ...
    * Longitudinal and experimental designs to examine interactions between facets of ST
