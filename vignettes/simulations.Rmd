...
res_2pl1 <- main_sim(n, items, 1000, parallel = TRUE, sim.2pl, .1)
res_2pl3 <- main_sim(n, items, 1000, parallel = TRUE, sim.2pl, .3)
res_2pl5 <- main_sim(n, items, 1000, parallel = TRUE, sim.2pl, .5)

# violation of unidimensionality
res_mult <- main_sim(n, items, 1000, parallel = TRUE, sim.xdim, Sigma = sigma, weights = weights)
```
...
## Simulation results
```{r}
err_2pl5 <- compute_errors(res_2pl5)
sm_2pl5 <- summarize(err_2pl5)
sm_2pl5
```

Based on our simulations, we can say that when $\sigma = .5$, infit statistics require us to remove approximately 20 percent of the items in small sample size scenarios ($n = 100$), 15 percent in mediam sample size scenarios ($n = 300$) and about 12 percent with large sample size ($n = 1000$).
...
err_mult <- compute_errors(res_mult)
summarize(err_mult)
```
...
#fit2pl <- mirt(dat, 4, itemtype = '2PL') # this doesn't converge ...
```
